<x-app-layout>
    <x-slot name="title">Daftar Lembaga</x-slot>

    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-bold text-bgray-900 dark:text-white">Daftar Lembaga</h2>
                <p class="text-sm text-bgray-600 dark:text-bgray-50">Kelola data master lembaga</p>
            </div>
            <a href="{{ route('lembaga.create') }}"
                ...
                <i class="fa fa-plus mr-2"></i> Tambah Lembaga
            </a>
        </div>
    </x-slot>
    ...
    <div class="mb-6 rounded-lg bg-white p-6 dark:bg-darkblack-600">
        <form method="GET" action="{{ route('lembaga.index') }}" class="grid gap-4 md:grid-cols-3">
            <div>
    ...
    <div class="md:hidden space-y-4">
        @forelse($lembaga as $l)
            <div class="mobile-card bg-white rounded-xl p-4 shadow-sm dark:bg-darkblack-600">
                <div class="mobile-card-header">
                    <div class="flex-1">
                        <a href="{{ route('lembaga.show', $l) }}" class="mobile-card-title text-bgray-900 hover:text-accent-300 dark:text-white">
                            {{ $l->nama }}
                        </a>
                        @if($l->alamat)
                            <p class="text-sm text-bgray-500 dark:text-bgray-400 mt-1">{{ Str::limit($l->alamat, 60) }}</p>
                        @endif
                    </div>
                    @if($l->is_active)
                ...
                <div class="mobile-card-body mt-3 space-y-2">
                    <div class="mobile-card-row">
                        ...
                        <span class="mobile-card-value">{{ $l->kota ?? '-' }}</span>
                    </div>
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Kendaraan</span>
                        <span class="mobile-card-value text-accent-400">{{ $l->kendaraan_dimiliki_count }} unit</span>
                    </div>
                    @if($l->telepon)
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Telepon</span>
                        <span class="mobile-card-value">{{ $l->telepon }}</span>
                    </div>
                    @endif
                </div>

                <div class="mobile-card-actions">
                    <a href="{{ route('lembaga.show', $l) }}"
        ...
        @empty
            <div class="rounded-xl bg-white p-8 text-center dark:bg-darkblack-600">
                <i class="fa fa-building mb-4 text-4xl text-bgray-300"></i>
                <p class="text-bgray-500 dark:text-bgray-400">Belum ada data lembaga</p>
                <a href="{{ route('lembaga.create') }}" class="mt-2 inline-block text-accent-300 hover:underline">
                ...
                <thead class="border-b border-bgray-200 dark:border-darkblack-400">
                    <tr class="text-left">
                        <th class="px-6 py-4 text-sm font-semibold text-bgray-600 dark:text-bgray-50">
                            <x-sortable-header column="nama" label="Nama Lembaga" :currentSort="$sortColumn" :currentDirection="$sortDirection" />
                        </th>
                        <th class="px-6 py-4 text-sm font-semibold text-bgray-600 dark:text-bgray-50">
                ...
                <tbody>
                    @forelse($lembaga as $l)
                        <tr class="border-b border-bgray-200 last:border-0 dark:border-darkblack-400">
                            <td class="px-6 py-4">
                                <a href="{{ route('lembaga.show', $l) }}" class="font-medium text-bgray-900 hover:text-accent-300 dark:text-white">
                                    {{ $l->nama }}
                                </a>
                                @if($l->alamat)
                                    <p class="text-sm text-bgray-500 dark:text-bgray-50">{{ Str::limit($l->alamat, 50) }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if($l->telepon || $l->email)
                                    @if($l->telepon)
                                        <span class="text-bgray-900 dark:text-white">{{ $l->telepon }}</span>
                                    @endif
                                    @if($l->email)
                                        <p class="text-sm text-bgray-500 dark:text-bgray-50">{{ $l->email }}</p>
                                    @endif
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center rounded-full bg-accent-50 px-3 py-1 text-sm font-medium text-accent-400">
                                    {{ $l->kendaraan_dimiliki_count }} unit
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($l->is_active)
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('lembaga.destroy', $l) }}"
                                        onsubmit="return confirm('Yakin ingin menghapus lembaga {{ $l->nama }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
